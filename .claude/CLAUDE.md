...
🔜 **Next session**:
- Phase 3 – Quality gates (Lighthouse CI, Playwright visual snapshots, Vitest on `lib/posts.ts`, a11y audit), **or**
- Phase 4 – Editorial extensions (formal "review" post *type* with artist/track header + `/recensioni` index; `<Score>` / `<EQGraph>` / harmony components).
